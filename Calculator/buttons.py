...
class ImageButton(CTkButton):
    def __init__(self, parent, func, col, row, image, text = '', color = 'dark-gray'): # text needs to be at the end as it is a default value
        super().__init__(
            master = parent,
            command = func, # function
            text = text,
...
class MathImageButton(ImageButton):
    def __init__(self, parent, operator, func, col, row, image, color = 'orange'):
        super().__init__(
            parent = parent,
            func = lambda: func(operator), # function
            image = image,
            col = col, 
            row = row, 
            color = color
